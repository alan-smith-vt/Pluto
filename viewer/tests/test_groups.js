...
// 10 shells with real ids 101..110
global.feaModel = {
  header: { nElements: 10 },
...
assert(gl.map(g => g.name).join() === 'WALL,C1,C2,BASE', 'four groups listed');
...
assert(gl[3].nodeCount === 3 && gl[3].count === 0, 'BASE is a node group (3 nodes, nothing painted)');
assert(FEAFeatures._resolved().unmatched === 0, 'node members are not counted as unmatched');
...
FEAFeatures._moveGroup(0, 4);            // to the end (index in original list)
FEAFeatures.refresh();
gl = FEAFeatures._groupList();
assert(gl.map(g => g.name).join() === 'C1,C2,BASE,WALL', 'WALL now last: ' + gl.map(g => g.name).join());
assert(gl[3].painted === 10 && gl[0].painted === 0, 'last group wins after reorder');
