...
def fig_dish(profiles: list[dict]) -> str:
    ...
    f.map(0.0, 1.0, 0.3, 1.05, x0, x1, y0, y1)
    ...
    f.text((x0 + x1) / 2, y0 + 32, "r / R  (R = shell radius; ring wall shaded)", anchor="middle", size=11, fill=MUTED)
    f.text(x0 - 60, y1 - 12, "w(r) / w_ref", size=11, fill=MUTED)
    rw_in = (R - 0.625) / R
    ...
    f.text(f.X(0.05), f.Y(0.86), "dashed: half-space, flexible circle, (2/π) E(r/R), 0.64 at the edge", size=10, fill=INK)
    f.text(f.X(0.05), f.Y(0.83), f"w_ref = (p + plate) / ks = {-plateau(profiles[0]) * 12:.3f} in", size=10, fill=MUTED)
    for (p, col) in zip(profiles, COLOURS):
        w0 = plateau(p)
