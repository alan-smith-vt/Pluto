...
public class RawViewerWriter
{
	...
	private static readonly string[] DispNames =
	{
	...
	// 	DSR & capacity field names derived from DSR enum so it can change overtime
	public static List<Component> BuildComponents(bool stress, bool displacements, 
		bool str, bool dsr)
	{
		if (!stress && !displacements && !dsr)
		{
		...
		if (stress)
		{
			...
			{
				string units = nameUnits(name);
				components.Add(new Component(name, "stress", units));
			}
		}
		if (displacements)
		{
		...
		if (str)
		{
			for (int i = 0; i < (int)STR.Count; i++)
			{
				...
				string units = nameUnits(name);
				components.Add(new Component(name, "str", units));
			}
		}
		if (dsr)
		{
    ...
    // // One design-strength record: per element corner, no load-case dimension.
    // // Values[] order matches the kind == "str" components in the order they
    // // appear in the constructor's single component list.
    ...
    public void AppendStresses(List<StressRecord> stresses)
    {
            ...
            0, 0, MemoryMappedFileAccess.ReadWrite))
        {
            for (int i = 0; i < stresses.Count; i++)
            {
				...
				if (plane == -1) { continue; } // bandaid to fix parse errors
