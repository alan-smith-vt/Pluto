...
if ($Skeleton) {
    [DuctEvaluate]::Skeleton((Resolve-Path $SectionsTsv).Path, $(if ($ForcesTsv) { (Resolve-Path $ForcesTsv).Path } else { $null }), $Material, [IO.Path]::GetFullPath($OutCsv), $(if ($CapacitiesCsv) { (Resolve-Path $CapacitiesCsv).Path } else { $null }), $CarbonSections)
}
if ($Evaluate) {
