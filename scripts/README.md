...
Entry point is `Section-Cut.ps1`; `lib/Config.ps1` loads every `.cs` under `lib/` and `arms/` in a
...
the sidecar writer, `arms/` the pipe / steel / combined CSV arms and the SAP2000 `.s2k` arm
(`SapToPluto.cs`, reuses `lib/readers/SapReader.cs`; usage notes in `vault/arms/`). The full damage table and
