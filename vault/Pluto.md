...
type: tool
status: wip
language: C# 5 / PowerShell 5.1 + JS (Three.js) + Python (SAP side)
url: https://github.com/alan-smith-vt/Pluto
local-path: D:\Notes\repos\Pluto
branch: sap-features
---
...
**Solves:** A FEMAP-style hub for finite-element data. Solver **arms** (pipe CSV, steel CSV, combined plant, SAP2000, STAAD post-processing) converge on one common model — a v4 binary for geometry and results plus a JSON **features sidecar** for everything a user defines — and one generic Three.js **viewer** consumes it. Superseded [[StressViewerDev]] and [[SapViewer]] (both archived) on 2026-09-03.

**Reach for it when:** You need to look at plant geometry from [[Voyager]]'s SQL extracts, or at STAAD/SAP2000 results, in a browser with groups, predicates, and section cuts; or you need to build a SAP2000 tank model from a TOML config and push its results straight to the viewer.

## How to run it

- Viewer: open `viewer/index.html`; drop a `.bin` and its `.features.json` together. No build, no server.
- Arms: dot-source `scripts/lib/Config.ps1` from Windows PowerShell 5.1 (one `Add-Type` batch), then `PipeToPluto`, `SteelToPluto`, `CombinedToPluto`, or `SapToPluto`. Usage per arm is in the repo vault's Arms map.
- SAP tank: `pythonTools/sap/run_sap.py` does TOML → SAP2000 (OAPI via comtypes) → `results.s2k` → `SapToPluto` → viewer. Real tank configs are gitignored.
- Documentation is the repo's own Obsidian vault; this note is its Home (maps below). Read [[vault/handoffs/handoff|handoff]] first.

## Limits & assumptions

- **C# 5 only, PowerShell 5.1 only, warnings as errors.** The production environment dictates it; `pwsh` is not production.
- Three STAAD lib files are throwing stubs pending re-hydration from the production side (DSR calculators, results post-processor, STAAD input builder).
- SAP builder: link, nonlinear-case, and frame tables in the `.s2k` are unverified against the SAP importer; settlement profiles not yet built; fluid is water only; no roof live load.
- Python is allowed only under `pythonTools/`.

## Used on

- [[<project>]] — the project task: tank builder (courses, baseplate, roof, gap support layer, ring wall, dents), SAP arm, viewer.
- Plant extraction with [[Voyager]]: pipe and steel CSVs → viewer.

Full detail: [[Pluto Deep Dive]]. Related: [[Structural Viewer Toolchain]].

## Repo vault (this note is its Home)
...
- **Hierarchy:** Home → map → note. Every note's first line after the frontmatter is a
  breadcrumb `*↑ [[vault/Pluto|Home]] › [[vault/<folder>/<Map>|<Map>]]*`; maps carry `*↑ [[vault/Pluto|Home]]*`.
