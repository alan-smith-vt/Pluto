...
| `ELEMENT FORCES - FRAMES` | beam `force` components `P (axial) V2 V3 T M2 M3` (frame local axes) from the first / last station of each frame per case; `run_sap.py` writes the table from `Results.FrameForce` |
| `ELEMENT FORCES - AREA SHELLS` | per-corner `stress` components `F11 F22 F12 M11 M22 M12 V13 V23` (those present), unit `kip/ft` or `kip-ft/ft`; labels carry the meaning in parentheses, e.g. `F11 (hoop membrane force)` in cylindrical mode, `F11 (local 1 membrane force)` otherwise (SAP default shell axes: 1 = horizontal = hoop on wall and roof, 2 = up the surface = meridional; the flat baseplate has 1 = X, 2 = Y) |
