...
- Scope: `-TopDirLike 'PACKAGE'` descends only into first-level folders whose name contains it; `-NameLike '*Calculation Report*'` keeps only PDFs so named; `-ExcludeDir` (default `Archive, Superseded, Old`) skips a PDF if any folder name under the root contains one of these. All case-insensitive. Non-packaged calcs: later, via a discovery pass that writes an explicit file list.
